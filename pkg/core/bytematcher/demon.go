...
*/
func (b *Matcher) demon(buf siegreader.Buffer, q chan struct{}, r chan<- core.Result) chan<- strike {
		...
		for _, v := range w {
			...
			for i, f := range kf {
				...
				if f.key.pMax == -1 || f.key.pMax+int64(f.key.lMax) > off {
					waitfor = true
				} else if hit, ok := hits[v]; ok && (hit.partials[i] != nil || hit.potentialIdxs[i] > -1) {
					waitfor = true
				}
	...
	// SIMPLE:
	// 1. Get a candidate strike from the list
	// 2. Test that strike
	// 3  Apply all hits, report results, quit if can, recalculate the list, return if the list is empty
	// 4. go back to 1.
	satisfy := func(st strike) {
		var hasPotential bool
		potentials := filterKF(b.tests[st.idxa+st.idxb].keyFrames(), waitSet)
		...
		if !hasPotential {
			// cache the strike
			s, ok := strikes[st.idxa+st.idxb]
			if !ok {
				s = &strikeItem{st, -1, nil}
				strikes[st.idxa+st.idxb] = s
			} else {
				if s.successive == nil {
					s.successive = make([][2]int64, 0, 10)
				}
				s.successive = append(s.successive, [2]int64{st.offset, int64(st.length)})
			}
			...
			for _, pot := range potentials {
				...
				hit.potentialIdxs[pot[1]] = st.idxa + st.idxb + 1
			}
			return
		}
		for {
			...
			var ok bool
			for _, pot := range potentials {
				st, ok = hits[pot[0]].nextPotential(strikes)
				if ok {
					break
				}
			}
